...
class EtherSendModule(DetectionModule):
    def __init__(self):
        super().__init__(
            name="CALL Ether Send",
            swc_id=UNPROTECTED_ETHER_WITHDRAWAL,
            hooks=["CALL"],
            description=(
                "Check for CALLs that send >0 Ether to either the transaction "
                "sender, or to an address provided as a function argument. If "
                "msg.sender is checked against a value in storage, check "
                "whether that storage index is tainted (i.e. there's an "
                "unconstrained write to that index)."
            ),
        )
